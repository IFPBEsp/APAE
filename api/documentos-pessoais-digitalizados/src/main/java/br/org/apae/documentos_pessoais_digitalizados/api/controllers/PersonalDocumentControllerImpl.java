...
public class PersonalDocumentControllerImpl implements PersonalDocumentController {
    ...
    public ResponseEntity<Void> attachPersonalDocument(@PathVariable("patientId") String patientId,
    ...
    @ApiResponses({ @ApiResponse(
        ...
        responseCode = "404",
        description  = "Documento ou paciente não encontrado"
    ) , @ApiResponse(
        responseCode = "500",
        description  = "Erro interno do servidor"
    ) })
    @DeleteMapping("/{patientId}")
    public ResponseEntity<Void> deleteDocument(@PathVariable("patientId") String patientId,
    ...
    public ResponseEntity<PersonalDocumentResDTO> listPersonalDocumentByType(
            @PathVariable("patientId") String patientId, @RequestParam("type") String documentType) {
        PersonalDocumentResDTO resDTO = this.personalDocumentService.listPersonalDocumentByType(patientId,
                                                                                                PersonalDocumentType.valueOf(
                                                                                                    documentType));
    ...
    @GetMapping(
        value        = "/{patientId}/vizualizar",
        produces     = MediaType.IMAGE_PNG_VALUE
    )
    public ResponseEntity<byte[]> viewPatientPersonalDocument(@PathVariable("patientId") String patientId,
                                                              @RequestParam("fileName") String fileName) {
