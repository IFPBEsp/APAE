...
public class TipoDeficienciaRepositoryImpl implements TipoDeficienciaRepository {
    ...
    @Autowired
    public TipoDeficienciaRepositoryImpl(TipoDeficienciaRepositoryJpa repository, TipoDeficienciaMapper mapper) {
    ...
    public Page<TipoDeficiencia> findAll(Pageable pageable) {
        ...
        List<TipoDeficiencia> content = page.getContent()
                .stream()
                .map(mapper::toDomain)
    ...
    public Page<TipoDeficiencia> findByDescricaoIgnoreCase(String descricao, Pageable pageable) {
        Page<TipoDeficienciaEntity> page = repository.findByDescricaoContainingIgnoreCase(descricao, pageable);
        List<TipoDeficiencia> content = page.getContent()
                .stream()
                .map(mapper::toDomain)
                .collect(Collectors.toList());
