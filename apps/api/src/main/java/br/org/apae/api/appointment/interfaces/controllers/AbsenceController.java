...
public interface AbsenceController {
        ...
        ResponseEntity<Page<AbsenceResponseDTO>> findAll(
                @RequestParam(required = false) @Schema(description = "ID do Agendamento Gerado (GeneratedAppointment) para filtro.") UUID generatedId,

                @RequestParam(required = false) @Schema(description = "ID do Paciente (Patient) para filtro.") UUID patientId,

                @RequestParam(required = false) @Schema(description = "ID do Profissional (Professional) para filtro.") UUID professionalId,
                Pageable pageable);

        @Operation(summary = "Justificar uma falta", description = "Justifica uma falta e a remove da contagem de faltas não justificadas.")
        @PatchMapping("/{id}/justify")
        ResponseEntity<AbsenceResponseDTO> justifyAbsence(
                @PathVariable UUID id,
                @RequestBody @Valid JustifyAbsenceDTO dto);
}