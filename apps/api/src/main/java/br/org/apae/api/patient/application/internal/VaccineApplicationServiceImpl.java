...
public class VaccineApplicationServiceImpl implements VaccineApplicationService {
    ...
    @Autowired
    public VaccineApplicationServiceImpl(
    ...
    @Override
    @Transactional
    public VaccineResponseDTO createVaccine(CreateVaccineDTO createVaccineDto) {

        if (vaccineRepository.existsByNameIgnoreCase(createVaccineDto.name())) {
            throw new VaccineConflictException();
        }

        Vaccine vaccine = vaccineMapper.toEntity(createVaccineDto);
    ...
    public List<VaccineResponseDTO> findAllVaccines() {
        ...
        return vaccines.stream()
                .map(vaccine -> {
                    ...
                    boolean inUse =
                            patientRepository.isVaccineInUse(vaccine.getId());
    ...
    @Override
    @Transactional(readOnly = true)
    public Set<VaccineResponseDTO> findVaccines(
            Set<VaccineNameDTO> vaccineNames) {

        Set<String> names = vaccineNames.stream()
                .map(VaccineNameDTO::name)
                .collect(Collectors.toSet());

        Set<Vaccine> vaccines =
                vaccineRepository.findByNameInIgnoreCase(names);
    ...
    @Override
    @Transactional(readOnly = true)
    public Set<VaccineResponseDTO> findVaccinesFromUpdateDTOs(
            Set<UpdateVaccineDTO> vaccineNames) {

        Set<String> names = vaccineNames.stream()
                .map(UpdateVaccineDTO::name)
                .collect(Collectors.toSet());
    ...
    @Override
    @Transactional
    public VaccineResponseDTO updateVaccine(
            UUID id,
        ...
        if (vaccineRepository.existsByNameIgnoreCaseAndIdNot(
                dto.name(),
                id)) {
