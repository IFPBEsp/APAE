...
public interface VaccineController {
        ...
        @Operation(summary = "Cadastrar Vacina", description = "Cria um novo registro de vacina no sistema. Falha se o nome já existir.", responses = {
            ...
            @ApiResponse(responseCode = "409", description = "Conflito: Vacina com este nome já existe.", content = @Content)
        })
        @PostMapping
        ResponseEntity<VaccineResponseDTO> createVaccine(@RequestBody CreateVaccineDTO dto);

        @Operation(summary = "Atualizar Vacina", description = "Atualiza o nome de uma vacina existente.", responses = {
        ...
        @Operation(summary = "Excluir Vacina", description = "Remove uma vacina pelo seu identificador (UUID).", responses = {
                @ApiResponse(responseCode = "204", description = "Vacina excluída com sucesso", content = @Content),
                @ApiResponse(responseCode = "404", description = "Vacina não encontrada", content = @Content),
                @ApiResponse(responseCode = "409", description = "Conflito: Vacina está vinculada a um ou mais pacientes.", content = @Content)
        })
        @DeleteMapping("/{id}")
