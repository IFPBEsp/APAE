...
class DisorderControllerTest {
 ...
 class Criacao {
  ...
  void shouldReturnConflictWhenCreateDisorderWithDuplicateName() throws Exception {
   ...
   when(disorderService.createDisorder(requestDto)).thenThrow(new DisorderConflictException(requestDto.name()));
 ...
 class BuscaEListagem {
  ...
  void shouldReturnNotFoundWhenSearchDisorderByIdNonExistent() throws Exception {
   UUID id = UUID.randomUUID();
   when(disorderService.findDisorderById(id)).thenThrow(new DisorderNotFoundException());
 ...
 class Atualizacao {
  ...
  void shouldReturnNotFoundWhenUpdateDisorderNonExistent() throws Exception {
   ...
   when(disorderService.updateDisorder(id, updateDto)).thenThrow(new DisorderNotFoundException());
  ...
  void shouldReturnConflictWhenUpdateDisorderWithDuplicateName() throws Exception {
   ...
   when(disorderService.updateDisorder(id, updateDto)).thenThrow(new DisorderConflictException(updateDto.name()));
