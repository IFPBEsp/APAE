...
import { createDisorderSchema, type CreateDisorderFormData } from "../disorders.schema";
import { useDisorderCreate } from "./use-disorder-create";

export function DisorderCreateForm() {
  const router = useRouter();
  const { createDisorder, isSubmitting } = useDisorderCreate();

  const form = useForm<CreateDisorderFormData>({
    resolver: zodResolver(createDisorderSchema),
    mode: "onChange",
    defaultValues: { name: "" },
  });

  const onSubmit = async (data: CreateDisorderFormData) => {
