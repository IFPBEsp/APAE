...
export async function fetchVaccineApi(id: string): Promise<Vaccine> {
  ...
  if (!response.ok) {
    ...
    throw new Error(
      errorData?.message || "Ocorreu um erro ao carregar a vacina.",
    );
  }
...
export async function createVaccineApi(
  ...
  if (!response.ok) {
    ...
    throw new Error(
      errorData?.message || "Ocorreu um erro ao criar a vacina.",
    );
  }
}
...
export async function updateVaccineApi(
  ...
  if (!response.ok) {
    ...
    throw new Error(
      errorData?.message || "Ocorreu um erro ao atualizar a vacina.",
    );
  }
}
