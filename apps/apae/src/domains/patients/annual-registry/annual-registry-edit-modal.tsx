      ...
      <DialogContent className="!max-w-[1200px] w-[95vw] h-[85vh] flex flex-col p-0 overflow-hidden bg-slate-50 rounded-xl shadow-xl">
                  ...
                  {isCreateMode && (
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem>
                          <FormLabel
                            className={`font-bold text-xs ${errors.year ? "text-red-500" : "text-slate-700"}`}
                          >
                            Ano de Referência
                          </FormLabel>
                          <Select onValueChange={field.onChange} defaultValue={field.value}>
                            <FormControl>
                              <SelectTrigger
                                className={`bg-slate-50 h-10 text-sm ${errors.year ? "border-red-500 ring-red-500" : "border-slate-200"}`}
                              >
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem>
                          <FormLabel
                            className={`font-bold text-xs ${errors.familyIncome ? "text-red-500" : "text-slate-700"}`}
                          >
                            Renda Familiar
                          </FormLabel>
                          <FormControl>
                  ...
                  <div className="space-y-3">
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem>
                          <FormLabel
                            className={`font-bold text-xs ${errors.diseases ? "text-red-500" : "text-slate-700"}`}
                          >
                            Doenças
                          </FormLabel>
                          <FormControl>
                            <div
                              className={errors.diseases ? "border-red-500 border rounded-md" : ""}
                            >
                              <StringMultiSelect
                                value={field.value || ""}
                                onChange={field.onChange}
                                placeholder="Digite doenças..."
                              />
                            </div>
                          </FormControl>
                          <FormMessage />
                    ...
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem>
                          <FormLabel
                            className={`font-bold text-xs ${errors.allergies ? "text-red-500" : "text-slate-700"}`}
                          >
                            Alergias
                          </FormLabel>
                          <FormControl>
                            <div
                              className={errors.allergies ? "border-red-500 border rounded-md" : ""}
                            >
                              <StringMultiSelect
                                value={field.value || ""}
                                onChange={field.onChange}
                                placeholder="Digite alergias..."
                              />
                            </div>
                          </FormControl>
                          <FormMessage />
                    ...
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem>
                          <FormLabel
                            className={`font-bold text-xs ${errors.continuousMedication ? "text-red-500" : "text-slate-700"}`}
                          >
                            Medicamentos
                          </FormLabel>
                          <FormControl>
                    ...
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem>
                          <FormLabel
                            className={`font-bold text-xs ${errors.vaccines ? "text-red-500" : "text-slate-700"}`}
                          >
                            Vacinas
                          </FormLabel>
                          <FormControl>
                            <GenericDatabaseSelect
                              value={field.value || []}
                              onChange={field.onChange}
                              endpoint="/apae-geral/api/vaccines"
                              labelSingular="Vacina"
                              labelKey="name"
                              placeholder="Selecione vacinas..."
                            />
                          </FormControl>
                          <FormMessage />
                        </FormItem>
                  ...
                  <div className="pt-1 space-y-3">
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem>
                          <FormLabel
                            className={`font-bold text-xs ${errors.disorders ? "text-red-500" : "text-slate-700"} mb-1.5 block`}
                          >
                            Transtornos
                          </FormLabel>
                          <FormControl>
                            <GenericDatabaseSelect
                              value={field.value || []}
                              onChange={field.onChange}
                              endpoint="/apae-geral/api/disorders"
                              labelSingular="Transtorno"
                              labelKey="name"
                              placeholder="Selecione transtornos..."
                            />
                          </FormControl>
                          <FormMessage />
                        </FormItem>
                    ...
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem>
                          <FormLabel
                            className={`font-bold text-xs ${errors.serviceTypes ? "text-red-500" : "text-slate-700"} mb-1.5 block`}
                          >
                          ...
                          <FormControl>
                            <GenericDatabaseSelect
                              value={field.value || []}
                              onChange={field.onChange}
                              endpoint="/apae-geral/api/service-types"
                              labelSingular="Tipo"
                ...
                {isCreateMode ? (
                  <div className="flex flex-col items-center justify-center h-full text-center p-8 opacity-60">
                    ...
                    <p className="text-slate-500 font-medium text-sm">
                      Documentos poderão ser anexados após a criação do registro.
                    </p>
                  </div>
                ) : (
                  <>
                    <div className="bg-slate-50 p-5 rounded-xl border-2 border-dashed border-slate-300 mb-6 group">
                      <label className="text-xs font-bold text-slate-500 uppercase mb-3 block text-center tracking-widest group-hover:text-[#0D4F97]">
                        Adicionar Novo Documento
                      </label>
                      <div className="flex flex-col gap-3 max-w-sm mx-auto w-full">
                        <Select value={docType} onValueChange={setDocType}>
                          <SelectTrigger className="w-full bg-white border-slate-200 h-10 text-sm">
                            <SelectValue />
                          </SelectTrigger>
                          <SelectContent>
                        ...
                        <Button
                          variant="outline"
                          className="w-full bg-white text-[#0D4F97] border-[#0D4F97]/20 hover:bg-[#0D4F97] hover:text-white h-10 transition-all text-sm"
                      ...
                      <div className="space-y-2">
                        {documents.length === 0 ? (
                          ...
                          documents.map((doc) => (
                            <div
                              key={doc.id}
                              className="group flex items-center justify-between p-3 bg-white border border-slate-100 rounded-xl shadow-sm hover:border-[#0D4F97]/20 transition-all duration-200"
                            >
                              <div className="flex items-center gap-3 overflow-hidden">
                                ...
                                <div className="flex flex-col min-w-0">
                                  ...
                                  <span className="text-[10px] text-slate-400 font-bold uppercase tracking-wide mt-0.5">
                                    {DOC_TYPE_TRANSLATIONS[doc.type] || doc.type}
                                  </span>
                                </div>
                              </div>
                              {doc.url && (
        ...
        <DialogFooter className="px-6 py-4 bg-white border-t border-slate-100 shrink-0 flex justify-end gap-3 z-10">
          <Button
            ...
            onClick={() => onClose()}
            type="button"
            className="text-slate-500 h-10 px-5 text-sm"
          >
            Cancelar
          </Button>
          <Button
