...
export async function GET(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    return NextResponse.json({ message: "Erro ao buscar documentos." }, { status });
  }
}
...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const response = await api.post(`/patients/${patientId}/documents`, backendFormData, {
      params: {
    ...
    return NextResponse.json({ message: "Erro no backend." }, { status });
  }
}
