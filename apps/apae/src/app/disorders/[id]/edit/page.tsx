...
export default function EditTranstornoPage() {
    ...
    if (id) {
      const fetchTranstorno = async () => {
        try {
          ...
          const errorMessage = error instanceof Error ? error.message : "Erro ao buscar transtorno.";
  ...
  const onSubmit = async (data: UpdateTranstornoDTO) => {
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : "Erro ao atualizar transtorno.";
      toast.error(errorMessage);
    }
  };
