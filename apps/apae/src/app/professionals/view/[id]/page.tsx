    ...
    <div className="w-full max-w-5xl mx-auto p-4 sm:p-6 md:p-10">
      ...
      <div className="flex flex-col items-center sm:flex-row sm:items-center gap-4 sm:gap-6 mb-10">
        <Avatar className="h-24 w-24 border-2 border-[#0D4F97]">
          <AvatarImage src={photoDoc?.url || ""} alt={date.name} />
          <AvatarFallback className="text-xl font-bold bg-[#B2D7EC] text-[#0D4F97]">
            {date.name?.charAt(0) || "P"}
          </AvatarFallback>
        </Avatar>
        <div className="text-center sm:text-left">
          <h2 className="text-xl font-bold text-[#0D4F97]">{date.name}</h2>
          <p className="text-base text-gray-600">{date.serviceArea?.area}</p>
        </div>
      </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 sm:gap-4">
              <div className="col-span-1 sm:col-span-2">
                ...
                <p className="text-base text-gray-700">
                  {date.professionalDocument || "—"}
                </p>
              </div>
              <div>
                <p className="font-semibold text-sm">Email</p>
                <p className="text-base text-gray-700">{date.email || "—"}</p>
              </div>
              <div>
          ...
          <CardContent className="space-y-2">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 sm:gap-4">
              <div className="col-span-1 sm:col-span-2">
                ...
                <p className="text-base text-gray-700">
                  {`${date.address.street}, ${date.address.number} - ${date.address.neighborhood}`}
                </p>
              </div>
              ...
              <div className="col-span-1 sm:col-span-2">
                ...
                <p className="text-base text-gray-700">
                  {date.address.complement || "—"}
                </p>
              </div>

              <div>
                <p className="font-semibold text-sm">Cidade</p>
                <p className="text-base text-gray-700">{date.address.city}</p>
              </div>

              <div>
                <p className="font-semibold text-sm">Estado</p>
                <p className="text-base text-gray-700">{date.address.state}</p>
              </div>

              <div>
                <p className="font-semibold text-sm">CEP</p>
                <p className="text-base text-gray-700">{date.address.cep}</p>
              </div>
            </div>
          </CardContent>
        </Card>
                  ...
                  <tbody>
                    {SHIFTS.map((shift) => (
                      <tr key={shift} className="border-t">
                        ...
                        {DAYS.map((day) => {
                          const cell = availabilityMatrix.find(
