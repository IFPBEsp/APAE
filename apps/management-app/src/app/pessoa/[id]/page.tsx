        ...
        <div className="flex flex-col items-center gap-y-4 w-full max-w-full mx-auto px-4 mt-6 mb-6">
            ...
            <Card className="w-full relative font-nunito text-[#0D4F97]">
                ...
                <CardContent>
                    ...
                    <p className="text-left">Renda familiar: <span className="text-[#000000]">R${pessoa?.cadastroAnual?.rendaFamiliar}</span></p>
                        ...
                        pessoa.responsaveis.map((resp: any, index: number) => (
                            <div key={index} className="mb-4 border-t border-gray-200 pt-2">
                                <p className="font-semibold text-left">
                                    {resp.tipoResponsavel ? resp.tipoResponsavel : `Responsável ${index + 1}`}
                                </p>
                                <p>Nome: <span className="text-[#000000]">{resp.nome}</span></p>
                                <p>Vivo: <span className="text-[#000000]">{resp.vivo}</span></p>
                                <p>Profissão: <span className="text-[#000000]">{resp.profissao}</span></p>
                                <p>RG: <span className="text-[#000000]">{resp.rg}</span></p>
                                <p>CPF: <span className="text-[#000000]">{resp.cpf}</span></p>
                                <p>Onde procurar: <span className="text-[#000000]">{resp.ondeProcurar}</span></p>
                                <p>Número para contato: <span className="text-[#000000]">{resp.emergencia}</span></p>
                            </div>
                        ))
                    ) : (
            ...
            <Card className="w-full relative font-nunito text-[#0D4F97]">
                ...
                <CardContent>
                    ...
                    {primeiroResponsavelVivo ? (
                        <>
                            <p>Quem procurar: <span className="text-[#000000]">{primeiroResponsavelVivo?.nome}</span></p>
                            <p>Onde encontrar: <span className="text-[#000000]">{primeiroResponsavelVivo?.ondeProcurar}</span></p>
                            <p>Contato de emergência: <span className="text-[#000000]">{primeiroResponsavelVivo?.emergencia}</span></p>
                        </>
                    ) : (
