...
export async function GET(
  ...
  try {
    ...
    const responsaveis =
      p.responsaveisResponses?.map((r: any) => ({
        id: r.id,
    ...
    const deficiencias =
      p.deficienciasResponses?.map((d: any) => ({
        id: d.id,
        descricao: d.descricao || "Não informado",
      })) ?? [];

    const atendimentos =
      p.atendimentosResponses?.map((a: any) => ({
        id: a.id,
    ...
    const cadastroAnual = p.cadastrosAnuaisResponses?.[0]
      ? {
          possuiBpc: p.cadastrosAnuaisResponses[0]
            .beneficioDePrestacaoContinuada
            ? "Sim"
            : "Não",
          doencas:
            p.cadastrosAnuaisResponses[0].historicoDoencas || "Não informado",
          alergias:
            p.cadastrosAnuaisResponses[0].historicosAlergias || "Não informado",
          medicacao:
            p.cadastrosAnuaisResponses[0].medicacoesContinuas ||
            "Não informado",
          rendaFamiliar:
            p.cadastrosAnuaisResponses[0].rendaFamiliar || "Não informado",
        }
      : null;
